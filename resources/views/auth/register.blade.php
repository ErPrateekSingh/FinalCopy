...
function field_error(id,text){
   ...
   if(id=="password"){$('#glyphcn'+id).attr('style', 'right: 20px !important;');}
   return false;
}
...
function ajaxEmailUnique(email) {
   ...
   $.ajax({
      ...
      success: function(data) {
         ...
         else valid = field_error("email","This Email aready Exists!");
      }
   });
   return valid;
}
...
function validateForm() {
   ...
   else if (!fname.match(/^[a-zA-Z ]+$/)) valid = field_error('fname','Only alphabets and spaces are allowed!'); else field_success('fname');
   if (lname == '') valid = field_error('lname','Last Name is required!');
   else if (!lname.match(/^[a-zA-Z ]+$/)) valid = field_error('lname','Only alphabets and spaces are allowed!'); else field_success('lname');
   if ($("#email").val() == '') valid = field_error('email','Email is required!');
   else if ($("#error_email").text() === "This Email aready Exists!") valid = false;
   ...
   else if (password.length <= 6) valid = field_error('password','Minimum 6 characters reqiured!'); else field_success('password');
   return valid;
}
...
$(document).ready(function(){
   ...
   $('#fname').on('blur',function(){($(this).val() == "")?field_clear("fname"):Validate('fname','a-s-only')});
   $('#lname').on('blur',function(){($(this).val() == "")?field_clear("lname"):Validate('lname','a-s-only')});
